"""최초 알고리즘(첫 버전) 스캔. 수정판·사이클 전략과 성과를 나란히 비교하려고 첫 커밋의 추천 규칙을 그대로 병행 운영한다.
...
수정판 스캔이 이미 받아 둔 일봉 캔들을 재사용하므로 추가 API 호출이 없다. 텔레그램으로는 알리지 않고 기록만 한다."""
